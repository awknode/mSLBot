...
ON *:START: {
  if (!$starting) {
    ...
    if (!$isfile(%lng)) || (!$file(%lng)) { wmm_dl $wmm_lang_url $qt(%lng) }
    if (!$isfile(%ico)) || (!$file(%ico)) { wmm_dl $wmm_main_ico_url $qt(%ico) }
    if (!$isfile(%png)) || (!$file(%png)) { wmm_dl $wmm_main_png_url $qt(%png) }
    if (!$isfile(%don)) || (!$file(%don)) { wmm_dl $wmm_donate_png_url $qt(%don) }
    ...
    if (%wmm_right_symb == $null) { set %wmm_right_symb $chr(8594) }
    ...
    if (%wmm_update) {
      ...
      if (%up == 3) { 
        ...
        if (%wmm_autoupdate_modules) { .timer[WMM_CHECK_FOR_UPDATE_MODULES_SILENT] -ho 1 3000 wmm_modules_silent_update }
        else { .timer[WMM_CHECK_FOR_UNSUPPORTED_MODULES] -ho 1 3000 wmm_modules_check_unsupported }
      }
    }
    else { wmm_input ok 60 The $upper($wmm_owner) Module Manager v $+ $wmm_ver ( $+ $wmm_crdate $+ ) by $wmm_owner has been installed successfully. | wmm_tool -s | wmm }
  }
  elseif ($starting) {
    ...
    var %png = $wmm_dir $+ no_preview.png
    var %don = $wmm_dir $+ donate.png
    if (!$isfile(%lng)) || (!$file(%lng)) { var %delay = 1 | wmm_dl $wmm_lang_url $qt(%lng) }
    if (!$isfile(%ico)) || (!$file(%ico)) { var %delay = 1 | wmm_dl $wmm_main_ico_url $qt(%ico) }
    if (!$isfile(%png)) || (!$file(%png)) { var %delay = 1 | wmm_dl $wmm_main_png_url $qt(%png) }
    if (!$isfile(%don)) || (!$file(%don)) { var %delay = 1 | wmm_dl $wmm_donate_png_url $qt(%don) }
    if (%wmm_lang == $null) { set %wmm_lang English }
    if (!%delay) { wmm_tool -s }
    ...
    .timer[WMM_CHECK_FOR_UPDATE_MODULES_SILENT] -ho 1 15000 wmm_modules_silent_update
...
ON *:DOWNLOAD:*: {
  if ($download == wmm_dl_images) {
    ...
    if ($downloaderr) || (!$isfile(%f)) || (!$file(%f)) { return }
  ...
  if ($download == wmm_mod_install) {
    ...
    if (!$dialog(%d)) { return }
    if (!$isfile(%f)) || (!$lines(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x033 | wmm_d_close %d | return }
  ...
  if ($download == wmm_mod_update) {
    ...
    if (!$dialog(%d)) { return }
    if (!$isfile(%f)) || (!$lines(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x030 | wmm_d_close %d | return }
    set -eu0 %wmm_signal_noclose 1
  ...
  if ($download == wmm_mod_silent_update) {
    ...
    if (!$isfile(%f)) || (!$lines(%f)) || (!%pos) { return }
    if ($wmm_ismod($nopath(%f))) { .unload -nrs $qt($nopath(%f)) }
    .load -rs $+ %pos $qt(%f)
  }
  if ($download == wmm_check_update_install) {
    ...
    var %f = $download($download).file
    if (!$isfile(%f)) || (!$lines(%f)) { $iif(%wmm_update !== 3,wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x020) | unset %wmm_update | return }
    var %lng = $wmm_dir $+ wmm_lang.ini
    var %ico = $wmm_dir $+ main.ico
    var %png = $wmm_dir $+ no_preview.png
    var %don = $wmm_dir $+ donate.png
...
ON *:DIALOG:wmm_module_sets:*:*: {
  ...
  if ($devent == close) {
    ...
    if (!$did(31)) || ($did(31) isalnum) { set %wmm_right_symb $chr(8594) }
...
ON *:DIALOG:wmm_module:*:*: {
  ...
  if ($devent == sclick) {
    ...
    if ($did == 600) {
      ...
      if ($isfile(%f_ico)) && ($file(%f_ico)) { did -vg $dname 28 $qt(%f_ico) }
      else { did -h $dname 28 }
    ...
    if ($did == 14) { 
      ...
      if ($isfile(%f)) && ($file(%f)) { wmm_pic $qt(%f) }
    }
    if ($did == 15) {
      var %m = $did(6).seltext
      if (!%m) { var %m = $did(60).seltext } 
...
alias wmm { 
  ...
  var %png = $wmm_dir $+ no_preview.png
  var %lng = $wmm_dir $+ wmm_lang.ini
  var %vbs = $wmm_temp $+ wmm_dl_images.vbs
  var %txt = $wmm_temp $+ wmm_images_urls.txt
  if ($isfile(%vbs)) && (!$wmm_isadi) { .remove $qt(%vbs) }
  if ($isfile(%txt)) && (!$wmm_isadi) { .remove $qt(%txt) }
  if (!$isfile(%lng)) || (!$isfile(%ico)) || (!$isfile(%png)) || (!$isfile(%don)) || (!$file(%lng)) || (!$file(%ico)) || (!$file(%png)) || (!$file(%don)) { var %delay = 1 }
  wmm_fix_extra_modules_installed
  if (%delay) {
...
alias -l wmm_check_before_open {
  ...
  var %ico = $wmm_dir $+ main.ico
  var %png = $wmm_dir $+ no_preview.png
  if (!$isfile(%lng)) || (!$file(%lng)) { wmm_input error 60 FATAL ERROR! @newline@ @newline@ $+ Error Code: 0x001 | return }
  if (!$isfile(%don)) || (!$file(%don)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x022 | return }
  if (!$isfile(%ico)) || (!$file(%ico)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x010 | return }
  if (!$isfile(%png)) || (!$file(%png)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x016 | return }
  dialog -md $1 $1
  return
  :error | wmm_werror $scriptline $error | reseterror
}
...
alias wmm_check_os {
  ...
  if (%adi64) { 
    ...
    if (!$isfile(%dll)) || (!$file(%dll)) { return It seems that missing an necessary DLL from your system while running under AdiIRC 64-bits, you have to install "tsc64.dll" from "https://goo.gl/4uNCPZ" website first and then try to install the project again. }
...
alias -l wmm_reset_images {
  ...
  if ($1) && ($1 == -i) && ($2) {
    ...
    while (%i <= $numtok(%imgs,44)) {
      if (%i == 1) { 
        ...
        if (!$isfile(%f)) || (!$file(%f)) { var %f = $wmm_dir $+ no_preview.png }
        if (!$isfile(%f)) || (!$file(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x026 | dialog -k $dname | return }
        did -g %d 14 $qt(%f)
      }
      if (%i == 2) { 
        var %f = $wmm_temp $+ $nopath($gettok(%imgs,%i,44))
        if (!$isfile(%f)) || (!$file(%f)) { var %f = $wmm_dir $+ no_preview.png }
        if (!$isfile(%f)) || (!$file(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x027 | dialog -k $dname | return }
        did -g %d 15 $qt(%f)
      }
      if (%i == 3) {
        var %f = $wmm_temp $+ $nopath($gettok(%imgs,%i,44))
        if (!$isfile(%f)) || (!$file(%f)) { var %f = $wmm_dir $+ no_preview.png }
        if (!$isfile(%f)) || (!$file(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x028 | dialog -k $dname | return }
        did -g %d 16 $qt(%f)
      }
      inc %i
    }
  ...
  did -h %d 28
  if (!$isfile(%f)) || (!$file(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x008 | wmm_d_close %d }
...
alias -l wmm_mod_install {
  ...
  var %f = $noqt($5-)
  if (!$isfile(%f)) || (!$lines(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x018 | wmm_d_close %d | return }
  set -eu0 %wmm_signal_noclose 1
  .load -rs $qt(%f)
...
alias -l wmm_mod_update { 
  ...
  var %f = $noqt($6-)
  if (!$isfile(%f)) || (!$lines(%f)) { wmm_input error 60 $wmm_lang(17) @newline@ @newline@ $+ $wmm_lang(18) 0x014 | wmm_d_close %d | return }
  set -eu0 %wmm_signal_noclose 1
  if ($wmm_ismod($nopath(%f))) { .unload -nrs $qt($nopath(%f)) }
  .load -rs $+ $2 $qt(%f)
...
alias -l wmm_mod_silent_update {
  var %f = $noqt($6-)
  if (!$1) || ($isid) || ($dialog(wmm_module)) || ($dialog(wmm_module_sets)) || ($4 !== S_OK) || ($wmm_error) || (!$isfile(%f)) || (!$lines(%f)) { return }
  if ($wmm_ismod($nopath(%f))) { .unload -nrs $qt($nopath(%f)) }
  .load -rs $+ $2 $qt(%f)
  return
  :error | wmm_werror $scriptline $error | reseterror
...
alias -l wmm_modules_list {
  ...
  if ($isfile($wmm_sets_file)) && ($lines($wmm_sets_file)) { jsonopen -f %v $qt($wmm_sets_file) }
...
alias -l wmm_modules_list_get {
  ...
  if (%images) { 
    hadd %h %mod $+ _images %images
    if (!$wmm_isadi) { wmm_get_images %images }
  }
  ...
  if (%x == $calc(%t -1)) { 
    ...
    if (!$wmm_isadi) { .timer[WMM_DOWNLOAD_IMAGES] -ho 1 4000 wmm_dl_images_now }
    else { .timer[WMM_DOWNLOAD_IMAGES] -ho 1 2000 wmm_dl_images_now }
...
alias -l wmm_get_images { 
  ...
  var %fw = $wmm_temp $+ wmm_images_urls.txt
  var %t = $numtok($1-,44)
  var %i = 1
  while (%i <= %t) {
    var %f = $gettok($1-,%i,44)
    ...
    if (!$isfile(%ft)) || (!$file(%ft)) || (%fd !== $date) { write $qt(%fw) %f }
...
alias -l wmm_dl_images_now {
  var %f = $wmm_temp $+ wmm_dl_images.vbs
  var %fx = $wmm_temp $+ wmm_images_urls.txt
  var %n = wmm_dl_images
  var %w = .fwrite -n %n
  ...
  if ($wmm_isadi) {
    ...
    if (%fd !== $date) { download -o %n $wmm_modules_images_url $qt(%zip) }
  }
  elseif (!$wmm_isadi) {
    if ($isfile(%fx)) && ($lines(%fx)) {

      .fopen -ox %n $qt(%f)

      %w Function ReplaceRegEx(origString,replaceString,replaceWith)
      %w Set TempRegEx=New RegExp
      %w TempRegEx.Pattern=replaceString
      %w TempRegEx.IgnoreCase=True
      %w ReplaceRegEx = TempRegEx.Replace(origString,replaceWith)
      %w End Function
      %w Set objFSO = CreateObject("Scripting.FileSystemObject")
      %w Set objDictionary = CreateObject("Scripting.Dictionary")
      %w currentDirectory = left(WScript.ScriptFullName,(Len(WScript.ScriptFullName))-(len(WScript.ScriptName)))
      %w UserInput = $qt($nopath(%fx))
      %w strHDLocationLinks = currentDirectory&""&UserInput
      %w If UserInput = "" Then
      %w Else
      %w If objFSO.FileExists(currentDirectory & UserInput) Then
      %w    ForReading = UserInput
      %w    Set objFile = objFSO.OpenTextFile(currentDirectory & UserInput,1)
      %w    i = 0
      %w        Do Until objFile.AtEndOfStream
      %w                strNextLine = objFile.Readline
      %w                If strNextLine <> "" Then
      %w                    objDictionary.Add i, strNextLine
      %w                End If
      %w                i = i + 1
      %w          Loop
      %w          objFile.Close
      %w          currentDirectory=Mid(currentDirectory,1,Len(currentDirectory)-1)
      %w          For Each strLine in objDictionary.Items
      %w                Set objFSO = CreateObject("Scripting.FileSystemObject")
      %w                linkNameSplit=Split(strLine,"/")
      %w                linkNameNumber=UBound(linkNameSplit)
      %w                linkNameTemp=""
      %w                For i = 3 To linkNameNumber-1
      %w                      linkNameTemp=linkNameTemp&"\"&linkNameSplit(i)
      %w                Next
      %w                strFileURL = strLine
      %w                strHDLocation = currentDirectory&"\"&linkNameSplit(linkNameNumber)
      %w                Set objXMLHTTP = CreateObject("MSXML2.SERVERXMLHTTP.6.0")
      %w                objXMLHTTP.open "GET", strFileURL, false
      %w                objXMLHTTP.send()
      %w                If objXMLHTTP.Status = 200 Then
      %w                      Set objADOStream = CreateObject("ADODB.Stream")
      %w                      objADOStream.Open
      %w                      objADOStream.Type = 1
      %w                      objADOStream.Write objXMLHTTP.ResponseBody
      %w                      objADOStream.Position = 0
      %w                      Set objFSO = Createobject("Scripting.FileSystemObject")
      %w                      If objFSO.Fileexists(strHDLocation) Then objFSO.DeleteFile strHDLocation
      %w                      Set objFSO = Nothing
      %w                      objADOStream.SaveToFile strHDLocation
      %w                      objADOStream.Close
      %w                      Set objADOStream = Nothing
      %w                  End if
      %w                Set objXMLHTTP = Nothing
      %w          Next
      %w          Set objFSO = Createobject("Scripting.FileSystemObject")
      %w          If objFSO.Fileexists(strHDLocationLinks) Then objFSO.DeleteFile strHDLocationLinks
      %w          If objFSO.Fileexists(WScript.ScriptFullName) Then objFSO.DeleteFile WScript.ScriptFullName
      %w    Else
      %w    End if
      %w End if
      ...
      if ($isfile(%f)) && ($lines(%f) == 60) { run -h $qt(%f) }
    }
  }
  return
  :error | wmm_werror $scriptline $error | reseterror
...
alias -l wmm_tool {
  ...
  if (s isincs $1) && (%wmm_tool) { 
    var %d = wmm_module
    var %ico = $wmm_dir $+ main.ico
    if (!$isfile(%ico)) || (!$file(%ico)) { wmm_dl $wmm_main_ico_url $qt(%ico) | return }
    wmm_t_close wmm
    wmm_t_close wmm1
    toolbar -as wmm1
...
alias -l wmm_modules_settings_list {
  ...
  if ($isfile($wmm_sets_file)) && ($lines($wmm_sets_file)) { jsonopen -fd %v $qt($wmm_sets_file) }
...
alias -l wmm_modules_all_installed_list {
  ...
  if (%n) && (%d) && (!$isalias(%d)) { return $iif($wmm_isadi && $isfile(%fm) && $file(%fm),$menuicon(%fm)) $style(2) $wmm_qd(%n $wmm_lang(76)) $+ :noop }
  else { return $iif($wmm_isadi && $isfile(%fm) && $file(%fm),$menuicon(%fm)) $iif($dialog(%d),$style(1)) $wmm_qd(%n $wmm_lang(76)) $+ : $+ %d }
  return
  :error | wmm_werror $scriptline $error | reseterror
}
...
menu @wmm {
  -
  $iif($wmm_isadi && $isfile($wmm_dir $+ main.ico) && $file($wmm_dir $+ main.ico),$menuicon($wmm_dir $+ main.ico)) $iif($dialog(wmm_module),$style(1)) $wmm_qd($iif($wmm_lang(73),$v1,Open the) $upper($wmm_owner) $iif($wmm_lang(16),$v1,Module Manager)): { .timer -ho 1 100 wmm | wmm_d_close wmm_module_sets }
  -
  $iif($wmm_isadi && $isfile($wmm_dir $+ main.ico) && $file($wmm_dir $+ main.ico),$menuicon($wmm_dir $+ main.ico)) $iif($dialog(wmm_module_sets),$style(1)) $wmm_qd($iif($wmm_lang(73),$v1,Open the) $upper($wmm_owner) $iif($wmm_lang(16),$v1,Module Manager) $iif($wmm_lang(69),$v1,Settings)): { .timer -ho 1 500 wmm_sets | wmm_d_close wmm_module }
  -
}

menu menubar,status,channel {
  $iif(%wmm_menu,-)
  $iif($istok(%wmm_menu,wmm,32),$iif($dialog(wmm_module),$style(1)) $iif($wmm_isadi && $isfile($wmm_dir $+ main.ico) && $file($wmm_dir $+ main.ico),$menuicon($wmm_dir $+ main.ico)) $wmm_qd($upper($wmm_owner) $iif($wmm_lang(16),$v1,Module Manager) $+ )): { wmm }
  $iif($istok(%wmm_menu,wmm_sets,32),$iif($dialog(wmm_module_sets),$style(1)) $iif($wmm_isadi && $isfile($wmm_dir $+ main.ico) && $file($wmm_dir $+ main.ico),$menuicon($wmm_dir $+ main.ico)) $wmm_qd($upper($wmm_owner) $iif($wmm_lang(16),$v1,Module Manager) $wmm_sep $iif($wmm_lang(69),$v1,Settings) $+ )): { wmm_sets }
  $iif($istok(%wmm_menu,wmm_mod_list,32) && $hget(WMM_MODULES_SUBMENU_INSTALLED),$iif($wmm_isadi && $isfile($wmm_dir $+ main.ico) && $file($wmm_dir $+ main.ico),$menuicon($wmm_dir $+ main.ico)) $wmm_qd($upper($wmm_owner) $iif($wmm_lang(16),$v1,Module Manager) $wmm_sep $iif($wmm_lang(77),$v1,Modules List)))
...
alias wmm_bel { return $iif(%wmm_right_symb,$v1,$chr(8594)) }
...
alias wmm_report {
  ...
  var %c = wmm_clone
  var %f = $wmm_temp $+ wmm_errors.log
  if (!$isfile(%f)) || (!$lines(%f)) || (!%wmm_help) { return }
...
alias wmm_tinycom {
  ...
  jsonopen -uw %v https://www.googleapis.com/urlshortener/v1/url?key= $+ $wmm_tiny_key $+ &longUrl= $+ $wmm_urlencode($1)
  if ($jsonerror) { jsonclose %v | return $1 }
  jsonhttpmethod %v POST
  jsonhttpheader %v User-Agent $chr(42) $+ $chr(47) $+ $chr(42)
  jsonhttpheader %v Content-Type application/json
  jsonhttpheader %v Connection Close
  jsonhttpfetch %v $chr(123) $+ $qt(longUrl) $+ : $qt($1) $+ $chr(125)
  var %url = $json(%v,id).value
  jsonclose %v
  if (%url) { return %url }
  elseif (!%url) { return $1 }
  return
