...
On $*:Text:/^(\+|-|!|@)(urban|ud) .*/Si:#: {
  ...
  if (%action isin +-) && ($regex($nick($chan,$nick).pnick,/(!|~|&|@)/)) {
    if (%action == +) {
      if ($istok(%SlangChanList,$+($network,$chan),32)) { .msg $chan [Urban Script] is already activated. }
      else { 
        .enable #Slang
        Set %SlangChanList $addtok(%SlangChanList,$+($network,$chan),32)
        .msg $chan [Urban Script] is activated in $chan .
      }
    }
    else {
      if (!$istok(%SlangChanList,$+($network,$chan),32)) { .msg $chan [Urban Script] is not currently activated. }
      else { 
        Set %SlangChanList $remtok(%SlangChanList,$+($network,$chan),1,32)
        .msg $chan [Urban Script] is deactivated in $chan . 
      }
    }
  }
  if (!$timer($+(Slang,$network,$nick))) && ($istok(%SlangChanList,$+($network,$chan),32)) {
...
On *:sockread:SlangUD*: {
  if ($sockerr) { echo -st Socket Error $nopath($script) | sockclose $sockname | return }
    ...
    elseif (<div id='not_defined_yet'> isin %SlangUD) { $svar($sockname,method) These niggers havent defined that shit yet. }
    elseif ($regex(%SlangUD,/<div\sclass=['"]ribbon)) { 
      ;    elseif ($regex(%SlangUD,/<div\sclass=['"]small['"]>/)) { 
    ...
    elseif ($regex(%SlangUD,/<title>Urban\sDictionary:\s(.*?)<\/title>/i)) { 
      $svar($sockname,method) 14(Urban Definition14) $censored($httpstrip($regml(1)))
    }
    elseif ($svar($sockname,num) == $svar($sockname,itemnum)) {
      ...
      if ($svar($sockname,div) == meaning) { 
        ...
        if (</div> isin %SlangUD) {
          put $svar($sockname,method) 14(Meaning14) $censored($replace($httpstrip($svar($sockname,definition)),$chr(13),$chr(32))) 
          svar $sockname div noop
        }
      }
      if ($svar($sockname,div) == example) { 
        ...
        if (</div> isin %SlangUD) {
          put $svar($sockname,method) 14(Example14) $censored($replace($httpstrip($svar($sockname,example)),$chr(13),$chr(32))) 
          svar $sockname div noop
        }
      } 
      if ($svar($sockname,div) == contributor) { 
        ...
        if (</div> isin %SlangUD) {
          put $svar($sockname,method) 14(Contributor14) $censored($replace($httpstrip($svar($sockname,contributor)),$chr(13),$chr(32))) 7:: $+(12,http://www.urbandictionary.com,$svar($sockname,url)))
          sockclose $sockname
          return
        }
      }
