...
alias twitch_stream_chan {
  ...
  if (!%id) { .msg $2 [ $+ $1 $+ ]: The $qt($3) stream channel is currently OFFLINE. }
  elseif (%id) { 
    .msg $2 [ $+ $1 $+ ]: Stream: $3 -=- Stream Status: Online -=- Stream Viewer(s): $iif(%viewers,$bytes($v1,bd),N/A) -=- Created on: $iif(%crdate,$v1,N/A) ( $+ $iif(%timeago,$v1,N/A) $+ ) 
    .msg $2 [ $+ $1 $+ ]: Channel Name: $iif(%name,$v1,N/A) -=- Game: $iif(%game,$v1,N/A) -=- Channel Status: $iif(%status,$v1,N/A) -=- Follower(s): $iif(%followers,$bytes($v1,bd),0) -=- Viewer(s): $iif(%views,$bytes($v1,bd),0) -=- Updated on: $iif(%updated,$v1,N/A) ( $+ $iif(%timeago_updated,$v1,N/A) $+ )
  }
  :end
  JSONClose %v
}
