...
alias -l Kin.BitCoin.Config.Networks { return HackTimes }
alias -l Kin.BitCoin.Config.Channels { return #internetz.me #UnderNix #insecurity }
...
alias -l Kin.BitCoin.Format {
  ...
  if (%type == 1) {
    ...
    if (%first == $true) { %frmt = %frmt symbol15m (Current Price $1 $+ ) }
    else { %frmt = %frmt 15m symbol (Current Price $1 $+ ) }
  ...
  elseif (%type == 2) {
    ...
    if (%first == $true) { %frmt = %frmt (Current Price) }
    else { %frmt = %frmt (Current Price) 15m }
