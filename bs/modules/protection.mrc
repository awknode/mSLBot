...
menu * {
  Flood Protection 4 Realz:dialog $iif($dialog(jpf2),-v,-dm) jpf2 jpf2
}
dialog -l jpf2 {
  title "bombass tuff productions (fuck you spammers) v.1"
  option dbu
  size -1 -1 205 268
  ;
...
alias -l chanlockdown {
  if (!$($+(%,chanlocked,$1),2)) {
    ...
    msg $1 $3- $+ . Alleged spammer:4 $+(,$2,,$chr(46)) 
    ...
    ; .timer 1 $calc($hget(protection,$+($1,ldf)) + 1) msg $1 Channel lockdown has been disabled.
    if ($2 ison $1) { 
      if ($hget(protection,$+(fpkb,$1)) == on) { ban -u $+ $calc($hget(protection,$+(fpkbl,$1)) * 60) $1 $address($2,2) Flooder | kick $1 $2 }
      else { raw -q kick $1 $2 spam }
    }
  }
  else { 
    if ($hget(protection,$+(fpkb,$1)) == on) { ban -u $+ $calc($hget(protection,$+(fpkbl,$1)) * 60) $1 $address($2,2) Flooder | kick $1 $2 }
...
on 1:load: {
  echo -a int protection enabled.
}
on 1:unload: {
  echo -a You are not protecting irc with int
}
