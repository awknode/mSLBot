...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wrt_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wrt_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wrt_prefix_nick $+ rutube) {
    ...
    $mod $+ _rutube_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wrt_prefix_chan $+ rutube) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _rutube_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wrt_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
