...
ON *:DIALOG:wip_sets:*:*: {
  if ($devent == init) {
    ...
    if (% [ $+ [ $mod ] $+ ] _prefix_chan) { did -ra $dname 5 % [ $+ [ $mod ] $+ ] _prefix_chan }
  ...
  if ($devent == close) {
    ...
    if (!$did(10)) || ($did(10) isalnum) { set % $+ $mod $+ _prefix_chan ! }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wip_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wip_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wip_prefix_nick $+ ipinfo) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$iptype($2)) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(58) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if ($2 == 127.0.0.1) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan $wmm_bold($nick) $+ ): $lang(59) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    $mod $+ _ipinfo_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  if ($1 == %wip_prefix_chan $+ ipinfo) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$iptype($2)) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(58) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if ($2 == 127.0.0.1) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan 12Don't use this address. | return }
    $mod $+ _ipinfo_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
alias wip_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wip_ipinfo_search {
  ...
  var %msg = .12ipinfo. → $iif(%country,$wmm_bold($lang(43)) $+ :6 %country $+  $iif(%country_code,6 $+ $chr(40) $+ $wmm_bold(%country_code) $+ $chr(41) $+ )) $iif(%region,$wmm_bold($lang(44)) $+ 12: %region $+ ) $iif(%isp,$wmm_bold($lang(45)) $+ 12: %isp $+ ) $iif(%org,$wmm_bold($lang(49)) $+ 12: %org $+ ) $iif(%city,$wmm_bold($lang(50)) $+ 12: %city $+ ) $iif(%reverse,$wmm_bold($lang(51)) $+ 12: %reverse $+ ) $iif(%as,$wmm_bold($lang(52)) $+ 12: %as $+ ) $iif(%zip,$wmm_bold($lang(53)) $+ 12: %zip $+ ) $iif(%timezone,$wmm_bold($lang(54)) $+ 12: %timezone $+ ) $iif(%map,$wmm_bold($lang(55)) $+ 12: $wmm_under(%map) $+ ) $iif(%type,$wmm_bold($lang(56)) $+ 12: %type $+ )
