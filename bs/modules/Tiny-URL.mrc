...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wtu_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wtu_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wtu_prefix_nick $+ tiny) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    var %output = $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg)
    if (!$2) { %output $nick ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($help_url)) $+ ) | return }
    if (!$wmm_isurl($2)) { %output $nick ( $+ $wmm_bold($nick) $+ ): $lang(44) - $lang(45) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($help_url)) $+ ) | return }
    %output $nick ( $+ $wmm_bold($nick) $+ ): $lang(46) $wmm_under($wmm_tinycom($2)) $+ .
  }
  if ($1 == %wtu_prefix_chan $+ tiny) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    var %output = $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg)
    if (!$2) { %output $chan ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($help_url)) $+ ) | return }
    if (!$wmm_isurl($2)) { %output $chan ( $+ $wmm_bold($nick) $+ ): $lang(44) - $lang(45) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($help_url)) $+ ) | return }
    %output $chan ( $+ $wmm_bold($nick) $+ ): $lang(46) $wmm_under($wmm_tinycom($2)) $+ .
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wtu_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
