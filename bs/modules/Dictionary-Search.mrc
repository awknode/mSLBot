...
dialog -l wds_sets {
  ...
  check "Show the 'Tags' information", 35, 160 58 164 10, tab 13
...
ON *:DIALOG:wds_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 35 $lang(47) $qt($lang(50)) $lang(48)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,tags,32)) { did -c $dname 35 }
...
ON *:LOAD: {
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title example tags link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wds_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wds_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wds_prefix_nick $+ dict) {
    ...
    $mod $+ _dict_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wds_prefix_chan $+ dict) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _dict_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wds_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wds_dict_search {
  ...
  var %tot = $json(%v,result_type).value
  if (!%tot) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  if ($istok(% [ $+ [ $mod ] $+ ] _show,tags,32)) { 
    var %z = 0
    while (%z < 5) {
      var %g = $json(%v,tags,%z).value
      if (%g) { var %tags = $addtok(%tags,%g,44) }
      inc %z
    }
    if (!%tags) { var %tags = 0 }
  }
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    if (% [ $+ [ $mod ] $+ ] _show_empty) {
      var %msg = 8,7 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title !== $null,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%example !== $null,$wmm_sep $wmm_bold($lang(44)) $+ :14 %example $+ ) $iif(%likes !== $null,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(53),$lang(51))) $+ :3 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%dislikes !== $null,$wmm_sep $wmm_bold($iif(%dislikes == 1,$lang(56),$lang(54))) $+ :4 $gettok($bytes(%dislikes,bd),1,46) $+ ) $iif(%tags !== $null,$wmm_sep $wmm_bold($iif(%tags == 1,$lang(52),$lang(50))) $+ :10 %tags $+ ) $iif(%uploader !== $null,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) {
      var %msg = 8,7 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%example,$wmm_sep $wmm_bold($lang(44)) $+ :14 %example $+ ) $iif(%likes,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(53),$lang(51))) $+ :3 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%dislikes,$wmm_sep $wmm_bold($iif(%dislikes == 1,$lang(56),$lang(54))) $+ :4 $gettok($bytes(%dislikes,bd),1,46) $+ ) $iif(%tags,$wmm_sep $wmm_bold($iif(%tags == 1,$lang(52),$lang(50))) $+ :10 %tags $+ ) $iif(%uploader,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
