...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wbp_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wbp_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wbp_prefix_nick $+ spell) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $nick ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under(goverment)) $+ ) | return }
    $mod $+ _spell_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wbp_prefix_chan $+ spell) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under(goverment)) $+ ) | return }
    $mod $+ _spell_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wbp_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wbp_spell_search {
  ...
  jsonhttpheader %v X-Mashape-Key $key
  ...
  var %msg = .12Spell Check Yo Shit. →  $iif(%cor_text,$wmm_bold($lang(43)) $+ 12: %cor_text $+ )
