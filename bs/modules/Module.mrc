...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wps_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wps_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wps_prefix_nick $+ wiki) {
    ...
    $mod $+ _wiki_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  if ($1 == %wps_prefix_chan $+ wiki) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($addon)) $+ ) | return }
    $mod $+ _wiki_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
...
alias wps_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wps_wiki_search {
  if (!$wmm_internet) || (!$1-) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (msg isin $3) { var %output = $3 $1 }
  if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show title link }
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    var %msg = .06Wiki. → $iif(%title,$wmm_bold($lang(43)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(44)) $+ : %desc $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(45)) $+ :6 $wmm_under(%url) $+ )
