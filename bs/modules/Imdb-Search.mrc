...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wis_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wis_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wis_prefix_nick $+ imdb) {
    ...
    $mod $+ _imdb_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wis_prefix_chan $+ imdb) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _imdb_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wis_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wis_imdb_search {
  ...
  jsonopen -ud %v https://api.themoviedb.org/3/search/movie?api_key= $+ $key $+ &query= $+ $wmm_urlencode($4-) $+ &search_type=ngram&page=1&include_adult=yes
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    if (% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 00.12 $+ $gettok($addon,1,45) $+ 00. → $iif(%title !== $null,$wmm_bold($lang(49)) $+ 12: %title $+ ) $iif(%desc !== $null,$wmm_sep $wmm_bold($lang(51)) $+ 12: %desc $+ ) $iif(%lang !== $null,$wmm_sep $wmm_bold($lang(58)) $+ 12: %lang $+ ) $iif(%adult !== $null,$wmm_sep $wmm_bold($lang(53)) $+ 12: %adult $+ ) $iif(%pop !== $null,$wmm_sep $wmm_bold($lang(54)) $+ 12: %pop $+ ) $iif(%votes !== $null,$wmm_sep $wmm_bold($iif(%votes == 1,$lang(43),$lang(55))) $+ 12: $gettok($bytes(%votes,bd),1,46) $+ ) $iif(%score !== $null,$wmm_sep $wmm_bold($lang(52)) $+ 12: %score $+ ) $iif(%crdate !== $null,$wmm_sep $wmm_bold($lang(56)) $+ 12: %crdate $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(57)) $+ 12: $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 00.12 $+ $gettok($addon,1,45) $+ 00. → $iif(%title,$wmm_bold($lang(49)) $+ 12: %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(51)) $+ 12: %desc $+ ) $iif(%lang,$wmm_sep $wmm_bold($lang(58)) $+ 12: %lang $+ ) $iif(%adult,$wmm_sep $wmm_bold($lang(53)) $+ 12: %adult $+ ) $iif(%pop,$wmm_sep $wmm_bold($lang(54)) $+ 12: %pop $+ ) $iif(%votes,$wmm_sep $wmm_bold($iif(%votes == 1,$lang(43),$lang(55))) $+ 12: $gettok($bytes(%votes,bd),1,46) $+ ) $iif(%score,$wmm_sep $wmm_bold($lang(52)) $+ 12:3 %score $+ ) $iif(%crdate,$wmm_sep $wmm_bold($lang(56)) $+ 12: %crdate $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(57)) $+ 12: $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
