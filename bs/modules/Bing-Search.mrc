...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wbs_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wbs_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wbs_prefix_nick $+ bing) {
    ...
    $mod $+ _bing_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wbs_prefix_chan $+ bing) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _bing_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wbs_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
...
alias wbs_bing_search {
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    var %msg = .12Bing. →  $iif(%title,$wmm_bold($lang(43)) $+ 12: %title $+ ) $iif(%desc,$wmm_bold($lang(44)) $+ 12: %desc $+ ) $iif(%url,$wmm_bold($lang(45)) $+ 12: $wmm_under(%url) $+ )
