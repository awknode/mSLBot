...
dialog -l wfb_sets {
  ...
  check "Show the 'ID' information", 30, 160 28 164 10, tab 13
...
ON *:DIALOG:wfb_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 30 $lang(47) $qt($lang(43)) $lang(48)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,id,32)) { did -c $dname 30 }
...
ON *:LOAD: {   
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show name id link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  .signal -n wmm_hash
  return
...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wfb_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wfb_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wfb_prefix_nick $+ facebook) {
    ...
    $mod $+ _facebook_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wfb_prefix_chan $+ facebook) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _facebook_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wfb_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wfb_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }
alias wfb_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
alias -l mod { return wfb }
...
alias wfb_facebook_search {
  ...
  var %v = facebook_search_ $+ $wmm_random

  jsonopen -ud %v https://graph.facebook.com/v2.10/search?q= $+ $wmm_urlencode($4-) $+ &type=user&access_token= $+ $key
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    var %id = $json(%v,data,%x,id).value
    if (!%id) { goto next }
    var %total = $calc(%total +1)

    if ($istok(% [ $+ [ $mod ] $+ ] _show,name,32)) { var %name = $json(%v,data,%x,name).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,id,32)) { var %name_id = %id }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) { 
      var %url = https://www.facebook.com/ $+ %id
      if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $wmm_tinycom(%url) }
    }

    var %msg = 0,12 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%name,$wmm_bold($lang(44)) $+ :6 %name $+ ) $iif(%name_id,$wmm_sep $wmm_bold($lang(43)) $+ :14 %name_id $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(45)) $+ :12 $wmm_under(%url) $+ )
...
#wfb_menu_status end
#wfb_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $isfile($scriptdir $+ $mod $+ _main.ico) && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wfb_menu_channel end
#wfb_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $isfile($scriptdir $+ $mod $+ _main.ico) && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wfb_menu_nicklist end
#wfb_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $isfile($scriptdir $+ $mod $+ _main.ico) && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
