...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wdm_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wdm_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wdm_prefix_nick $+ dailymotion) {
    ...
    $mod $+ _dailymotion_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wdm_prefix_chan $+ dailymotion) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    ...
    $mod $+ _dailymotion_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wdm_owner { return $+($chr(119),$chr(101),$chr(115),$chr(116),$chr(111),$chr(114)) }
