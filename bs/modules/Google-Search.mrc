...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _prefix_chan == $null) { set % $+ $mod $+ _prefix_chan $chr(36) }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs(%wgs_prefix_nick,\E,\E\\E\Q) $+ \E|\Q $+ $replacecs(%wgs_prefix_chan,\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if ($1 == %wgs_prefix_nick $+ google) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $nick ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($ [ $+ [ $mod ] $+ ] _owner)) $+ ) | return }
    $mod $+ _google_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,notice,.notice) $2-
  }
  if ($1 == %wgs_prefix_chan $+ google) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $chan ( $+ $wmm_bold($nick) $+ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($ [ $+ [ $mod ] $+ ] _owner)) $+ ) | return }
    $mod $+ _google_search $nick $chan $iif(% [ $+ [ $mod ] $+ ] _output,msg,.msg) $2-
  }
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
}
...
alias wgs_google_search {
  if (!$wmm_internet) || (!$1-) || (!$key) || (!$cx) { return }
  ...
  jsonopen -ud %v https://www.googleapis.com/customsearch/v1?key= $+ $key $+ &cx= $+ $cx $+ &q= $+ $wmm_urlencode($4-) $+ &filter=1&start=1&num= $+ % [ $+ [ $mod ] $+ ] _max_results
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    var %msg = .12Google. → $iif(%title,$wmm_bold($lang(43)) $+ 12: %title $+ ) $iif(%desc,$wmm_bold($lang(44)) $+ 12: %desc $+ ) $iif(%url,$wmm_bold($lang(45)) $+ 12:12 $wmm_under(%url) $+ )
